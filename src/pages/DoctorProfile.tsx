...
const DoctorProfile = () => {
  ...
  useEffect(() => {
    const checkOwnership = async () => {
      if (id && user) {
        const isOwner = await isUserDoctorOwner(id, user.id);
        setIsDoctorOwner(isOwner);
      }
    };
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Header />
        <div className="container max-w-6xl py-6 px-4 md:px-0">
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        {isLoading ? (
          ...
          <>
            <div className="flex justify-between items-center mb-6">
              ...
              {isUserDoctor && (
                <DoctorNotificationsPanel doctorId={id!} isDoctorOwner={isUserDoctor} />
              )}
            </div>
      ...
      <footer className="py-8 px-6 bg-gray-50 mt-auto">
        <div className="max-w-7xl mx-auto text-center text-gray-500 text-sm">
          &copy; {new Date().getFullYear()} Doctal HealthCare Directory. All rights reserved.
        </div>
      </footer>
    </div>
  );
